...
export function MangaCard({ manga }: { manga: MangaCardData }) {
  return (
    <Link href={`/manga/${manga.slug}`} className="group block">
      <div className="shine-surface shine-hover tilt-card relative aspect-[2/3] overflow-hidden rounded-lg border border-border bg-surface shadow-[inset_0_1px_0_0_rgb(255_255_255/0.06)] group-hover:border-primary/50">
        {manga.coverUrl ? (
          <Image
            src={manga.coverUrl}
      ...
      <h3 className="mt-1.5 line-clamp-2 font-heading text-sm font-semibold leading-tight text-foreground transition-colors group-hover:text-highlight">
        {manga.title}
      </h3>
    </Link>
  );
...
export function MangaCardSkeleton() {
  return (
    <div>
      <div className="aspect-[2/3] animate-pulse rounded-lg border border-border bg-surface" />
